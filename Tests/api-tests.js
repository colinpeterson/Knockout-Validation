...
QUnit.test('Nested Grouping works after changing view model - Observable', function(assert) {
	var vm = {
		one: ko.observable().extend({ required: true }),
		two: {
			one: ko.observable().extend({ required: true })
...
QUnit.test('validatedObservables updates validation state when its value changes', function(assert) {
	...
	assert.isFalse(identity.isValid(), 'observable is not valid');
...
QUnit.test('validatedObservables updates validation state when its value changes', function(assert) {
	...
	assert.isTrue(identity.isValid(), 'observable is valid');
...
QUnit.test('validatedObservable works when changing from null to object', function(assert) {
	...
	assert.isTrue(testObj.isValid(), 'observable is valid');
	...
	assert.isFalse(testObj.isValid(), 'observable is not valid');
